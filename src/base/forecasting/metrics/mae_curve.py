...
def compute_mae_curve(observations: np.ndarray, forecasts: List[Tuple[int, np.ndarray]]) -> np.ndarray:
    """
    Computes the Mean-Absolute-Deviation (MAE) as a function of n_samples_ahead based on a series of forecasts.

    The resulting MAE-curve is an indicator of how good the forecasts are as a function of how far ahead we're forecasting.
    ...
    :return: 1D numpy array with MAE curve (as long as the longest forecast),
    ...
    # compute MAE curve
    mae_curve = np.zeros(len(devs))
    for n, devs_group in devs.items():
        mae_curve[n] = np.mean(devs_group)
