...
class CrossValidationPlot1D:
    ...
    def __init__(self, param_names: List[str], data: List[Tuple[Tuple, "CVResult"]]):
        ...
        # misc
        self.error_bounds = ErrorBounds.QUARTILES
    ...
    def create(self) -> Tuple[plt.Figure, plt.Axes]:
        ...
        training_loss_mean = np.array([cv_result.train_metric_mean for _, cv_result in self.data])
        validation_loss_mean = np.array([cv_result.val_metric_mean for _, cv_result in self.data])

        training_loss_lb = np.array([self.error_bounds.get_lb(cv_result.train_metrics) for _, cv_result in self.data])
        training_loss_ub = np.array([self.error_bounds.get_ub(cv_result.train_metrics) for _, cv_result in self.data])
        validation_loss_lb = np.array([self.error_bounds.get_lb(cv_result.val_metrics) for _, cv_result in self.data])
        validation_loss_ub = np.array([self.error_bounds.get_ub(cv_result.val_metrics) for _, cv_result in self.data])
        ...
        # --- actual lines ---
        h_train = ax.plot(self.x_values, training_loss_mean, "r-x")
        h_val = ax.plot(self.x_values, validation_loss_mean, "g-x")

        # --- set limits ---

        # ideally, center median of validation losses upper bound
        y_max_ideal = 2 * np.median(validation_loss_ub)

        # make sure minimum validation loss is still discernible
        y_max = min(10 * min(validation_loss_mean), y_max_ideal)
